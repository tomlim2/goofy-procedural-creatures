...
| ACTION AUTO / IDLE / 행위 | 그 종족 전원에게 행위 하나를 강제 — 두발(인사·만세·팔짱·경례…), 네발(앞발 들기·긁기·꼬리 흔들기). IDLE은 행위 없이 기본 모션만 |
...
19슬롯 108파츠. 슬롯은 형태만 담고, 자세·동작은 모션이다. 길이·체격(`armLength` `legLength` `build`)은 형태와
