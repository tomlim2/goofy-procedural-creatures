...
| `src/stroke.js` | Strokes → ribbon geometry. Wobble, pressure, scribbles, scribble fills, hatching. `pencil()` — the reference's line kept next to `stroke()`, its numbers in `PENCIL`. `MATERIALS` + `paint()` — what a surface is made of: how its area is filled, as a base layer (flat, wash) and texture layers (hatch, scratch, bloom, dab, speckle, band). `GOOFY_OUTLINES` + `contour()` — the goofy outline (RIBBON, PENCIL). `GOOFY_FUR` + `fur()` — the goofy fur, how hair is grown (SCRIBBLE). A part names them (the board: FLAT, PENCIL, SCRIBBLE) instead of picking techniques and widths. `buildGeometry` (several sketches → one geometry) | [drawing](guidelines/drawing.md) |
