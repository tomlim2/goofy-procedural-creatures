...
└── headGroup                ← 원점 = 목(neckY = bodyTop). 갸웃·롤·끄덕·딥. 윤곽만 여기 직접
    ...
    ├── crownGroup           ← 머리에 **붙는** 것. 얼굴 돌림 때 이목구비 이동량의 [0.45, 0.3]배만큼 같은 방향으로 밀린다(시차, 크기 그대로)
    ...
    └── faceGroup            ← 원점 = 머리 중심(headCy). 얼굴 돌림으로 x/y 이동 + 눌림. 이목구비 전부
        ├── faceFrame ×3         ← 보일 변형. 눈(정지)·볼·수염 채색(2.3) + 잉크(2.4)
...
| 2.1 | 머리 앞 채색 — 개 귀·고양이 귀·모자. 불투명 (윤곽·머리카락·뿔 밑동을 덮는다 — 귀가 실루엣의 혹으로 붙는다) (crownGroup) |
| 2.2 | 머리 앞 잉크 |
...
| 6 | 눈 ^^ 아치 |
| 6.4 | 얼굴 맨 앞 채색 (주둥이) |
| 6.5 | 얼굴 맨 앞 잉크 (코·안경) — 놀라 커진 흰자·눈꺼풀이 코·안경테를 덮지 못하게 |
...
- **crownGroup** — headGroup과 같은 원점(목). 얼굴 돌림 때 position만 (x 0.45배 · y 0.3배). scale은 건드리지 않는다 — 부속물은 자리만 옮기지 크기가 변하지 않는다
