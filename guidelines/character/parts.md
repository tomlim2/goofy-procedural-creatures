...
| lidded | 무거운 눈꺼풀 — 큰 **흰자** + 윤곽 + 흰자를 가로지르는 **굵고 처진 눈꺼풀 선**(가운데가 처진다) + 그 밑에서 내다보는 동공. 반감김(half)이 획 하나라면 이건 흰자가 있는 눈이다 | ✗ |
...
흰자가 있는 정적 눈(hollow·lidded·sharp·soft)의 선·동공은 **어두운 팔레트 잉크**다 — 흰자 위라 밝은 얼굴 잉크로 그리면 묻힌다 (도깨비도 같다).
...
인상만 바뀌어야 한다. 눈 안은 **늘 흰자**고 잉크는 **윤곽·눈꺼풀 선·동공**에만 들어간다: 눈꺼풀을 먹으로 채우면 검은 머리(도깨비·검은 털)에서
머리와 한 덩어리가 되어 **흰 초승달만 남아** 눈으로 안 읽힌다. 눈꺼풀 선 굵기는 눈 크기에 비례한다(≤ 0.2r) — 고정 굵기면 작은 눈(고양이)에서 흰자를 다 덮는다.
