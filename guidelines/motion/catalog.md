...
| 상태 | `states.js` | **기본 상태(mode)** idle·sleep·walk · ^^ 행복 눈 · 윙크 · 갸웃 · 눈썹 상태 · 입 상태 · 둘러보기(얼굴 돌림 유지) · 행위 예약(팔·몸·네발 층 각각 — 언제 어떤 행위를) |
...
| tailAngle / tailTip / tailPuff / tailRaise · tailRaiseStyle | rad / rad / 0~1 / 0~1 · "straight"·"hook" | 뿌리 마디 회전 / 끝 마디 회전(뿌리 기준) / tailGroup.scale(1 + 0.3·puff) / 관절마다 쉼→세움 목표각 섞기 (scene/animate.js) |
...
| **얼굴 돌림** (좌우·위아래) | 시선 추종, 임계감쇠 w 0.1 (≈ 0.8초). 두피 위 머리카락·모자·뿔(crownGroup)은 이목구비 이동량의 45%(x)·30%(y)만 같은 방향으로, 귀(earGroup)는 40%(x)·15%(y) **반대 방향**으로, 앞머리·뒷머리(hairGroup)는 둘 다 12%(x)·8%(y)만(거의 고정, 같은 비율) 밀린다 — 위치만, 크기 그대로 | yaw 배율 human 0.5 / pup 0.7 / cat 0.8 / imp 0.6 (위아래는 ×0.6) | 이목구비 전체(눈·코·입·눈썹·안경·볼·수염·주둥이)가 머리 안에서 밀리고 살짝 눌린다. 동공이 먼저 가고 얼굴이 뒤따른다 |
...
목표 자세로 섞는다 — 골격이 말렸든 뒤로 뻗었든 상관없이 **띡 곧게 선다**(straight: 전부 위, hook: 끝 두 마디가 머리 쪽으로 굽음).
...
| **채찍질** lash | 놀람에 딸림 | — | **금지**(`lash: 0`) — 고양이가 꼬리를 치는 모션은 없다. 코드는 남아 있되 확률 0 |
| **세움** raise | 기분 좋음에 딸림 | — | ♥ 놀람 시작·♥ 이모지 때: `tailRaise` 0→1(0.4초), 3~5초 유지, 0.6초에 내림. 관절 목표각으로 골격과 무관하게 곧게 선다. 두 가지(반반) — **straight** 빳빳이 위(뿌리는 살짝 뒤로 젖혀 엉덩이에서 곧장) / **hook** 끝 두 마디가 앞(머리 쪽)으로 굽고 끝 ±0.08 12Hz 떨림 |
| **곤두섬** puff | 놀람에 딸림 | — | 놀란 동안 꼬리 **굵기**만 1 → 1.6배 (`tailPuff`, 길이 그대로). 타이밍은 사람 눈이 깜짝 놀랄 때의 법칙 그대로 — 동공과 **같은 봉투**: 0.1초에 곤두서고 3.8초 유지, 0.1초에 가라앉음 |
