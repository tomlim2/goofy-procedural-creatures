...
| `src/stroke.js` | Strokes → ribbon geometry. Wobble, pressure, scribbles, scribble fills, hatching. `pencil()` — the reference's line kept next to `stroke()`, its numbers in `PENCIL`. `MATERIALS` + `paint()`/`mark()` — what a surface is made of, as channels (fill, line); a part names a material (GRAPHITE: the head and body, FLAT: patches) instead of picking fills and widths. `buildGeometry` (several sketches → one geometry) | [drawing](guidelines/drawing.md) |
