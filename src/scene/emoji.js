...
import { POPS, INKS, MARKS } from "../character/vocabulary/palette.js";
...
export function buildEmoji(kind, noise) {
  ...
  if (kind === "heart") {
    ...
    sketch.fill(bar, INKS[0]);
    sketch.fill(blobPath(0, -0.012, 0.011, 0.011, { lumps: 3, amount: 0.15, noise: null }), INKS[0]);
  } else if (kind === "zzz") {
    // "z z" — sleep. One big z with a small z up and to the right. Short strokes are sampled densely to get past the end taper
    sketch.line([[-0.026, 0.028], [0.014, 0.028], [-0.026, -0.006], [0.014, -0.006]], { color: INKS[0] });
    sketch.line([[0.02, 0.06], [0.044, 0.06], [0.02, 0.04], [0.044, 0.04]], { color: INKS[0], size: "S" });
  } else if (kind === "sweat") {
    ...
    for (let i = 0; i < 3; i += 1) {
      const x = -0.03 + i * 0.03;
      sketch.fill(blobPath(x, 0.012 + (i % 2) * 0.006, 0.009, 0.009, { lumps: 3, amount: 0.15, noise: null }), INKS[0]);
    }
  } else {
    ...
    sketch.line(arcPath(0, 0.045, 0.03, 0.03, Math.PI, -Math.PI * 0.35, 16), { color: INKS[0] });
    sketch.fill([[0.004, 0.016], [0.018, 0.014], [0.013, -0.01], [0.001, -0.008]], INKS[0]);
    sketch.fill(blobPath(0.007, -0.03, 0.011, 0.011, { lumps: 3, amount: 0.15, noise: null }), INKS[0]);
  }
