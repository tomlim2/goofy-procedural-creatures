...
| `src/stroke.js` | Strokes → ribbon geometry. Wobble, pressure, scribbles, scribble fills, hatching. `pencil()` — the reference's line kept next to `stroke()`, its numbers in `PENCIL`. `MATERIALS` + `paint()` — what a surface is made of: how its area is filled, as channels — a base color (flat, with the part's pattern and decals in it) and its texture (hatch, scratch, dab, speckle) at a value step (`VALUES`: black · hatch · scribble · stipple · light, from the part's darkness and the creature's hand). `GOOFY_OUTLINES` + `contour()` — the goofy outline (RIBBON, PENCIL). `GOOFY_FUR` + `fur()` — the goofy fur, how hair is grown (SCRIBBLE). A part names them (the board: FLAT, PENCIL, SCRIBBLE) instead of picking techniques and widths; everything a creature fills takes its material through `paintPart` (details at their own color's step). `buildGeometry` (several sketches → one geometry) | [drawing](guidelines/drawing.md) |
