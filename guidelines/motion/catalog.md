...
| **얼굴 돌림** (좌우·위아래) | 시선 추종, 임계감쇠 w 0.1 (≈ 0.8초). 머리카락·모자·귀·뿔(crownGroup)은 이목구비 이동량의 45%(x)·30%(y)만 같은 방향으로 밀린다 — 위치만, 크기 그대로 | yaw 배율 human 0.5 / pup 0.7 / cat 0.8 / imp 0.6 (위아래는 ×0.6) | 이목구비 전체(눈·코·입·눈썹·안경·볼·수염·주둥이)가 머리 안에서 밀리고 살짝 눌린다. 동공이 먼저 가고 얼굴이 뒤따른다 |
