...
| 상태 | `states.js` | 반감김 · ^^ 행복 눈 · 윙크 · 갸웃 · 눈썹 상태 · 입 상태 · 팔 자세 |
...
그 자세를 기본으로 두다가 12~36초마다 다른 자세로 2~6초 갔다 돌아온다.

팔은 **두 관절**이다 — 어깨(pivot)와 팔꿈치(elbow). 자세마다 [어깨각, 팔꿈치각] 쌍이고,
팔꿈치각이 0이 아니면 접힌다. `armPoseAngles(pose, side)`.

| 자세 | 어깨 (outward) | 팔꿈치 | 인상 | 쉼 자세 가중치 |
| --- | --- | --- | --- | --- |
| **tpose** | 1.57 | 0 | 수평으로 곧게. **두발의 기본** | 6 |
| rest | 0.35 | 0.35 | 살짝 굽혀 늘어뜨림 | 1.5 |
| behind | −0.2 | 0 | 뒷짐 (back 스케치) | 1.5 |
| hips | 0.55 | 1.7 | 허리에 손. 팔꿈치 크게 접음 | 1 |
| out | 1.35 | −0.15 | 벌림 | 0.5 |
| up | 2.4 | −0.5 | 만세 | 전환 시만 |
| cross | 0.35 | 2.0 | 팔짱. 아래팔이 몸 앞으로 | 전환 시만 |

전환 후보 가중치: hips 2 / tpose·rest·out·behind·cross 1.5 / up 0.7.
front/back 전환은 어깨각이 기준 0.35rad 이내로 돌아온 뒤에만 한다. 회전 중 바꾸면 튄다.
매우 긴 팔(verylong)은 rest를 후보에서 뺀다 — 늘어뜨리면 바닥을 뚫는다.
팔꿈치는 자세각 + 지터의 절반을 받아 관절이 따로 끓는다.
