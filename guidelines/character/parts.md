...
| cat | pointy(기본) · pointyMid(좁고 긴) · pointyBig(넓고 큰, 폭보다 높이가 크다) | **정수리 양쪽 모서리(~35°, 네모 머리는 조금 안쪽 30°)의 채운 세모 혹** — 밑변을 윤곽 안으로 넣어 머리와 한 덩어리, 머리와 같은 굵기·2회 윤곽, 거의 수직에 아주 살짝만 바깥(0.05~0.1rad, 좌우 다르게), 끝은 뭉툭. 안쪽 귀는 개체별(이중선 60% · 어둡게 채움 15% · 없음). round·fold·flap·long·none 없음(레퍼런스에 없다) |
