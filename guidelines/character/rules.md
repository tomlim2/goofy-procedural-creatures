...
- **An eye that one stroke defines is left-right symmetric.** For droop, half and side, and for any eye a lid pupil closes (sleepy, line, happy), `eyeGeometry` sets the size and height skews (`eyeSizeSkew`, `eyeHeightSkew`) to 0 — one lid line or arch defines the eye, and if only one side is smaller or higher it reads as a mistake rather than "a smaller eye". Mismatched eyes are only for open eyes an outline defines (ring, wide, oval, hollow, the lidded set, slit) (`layout.js LINE_EYES`, `slots.js LID_PUPILS`)
