...
export const SLOTS = {
  ...
  // **Hair is two slots**, combined freely (draw/hair.js): the FRONT (앞머리 — what sits on the head itself, in front of it) and
  // the BACK (뒷머리 — what hangs behind and beside the head). Every piece is filled. hairFront stands where the one `hair` slot
  // stood, so the roll's count is unchanged; hairBack is a late slot (the end of LATE_SLOTS). The 26 old values map onto the
  // two, and onto headgear for a bun or an apple top (character/file.js OLD_HAIR)
  // The front is everything that sits on the head itself, in front of it: the fringes (hairline — the cap itself down to a straight
  // hairline, no piece of its own · blunt · swept · curtain · sideLock), the crown cap alone (cap), the mohawk, the tufts, the
  // curls and the two hoods; the spiked rings (spikes · hedgehog) stand round the head from behind it and are backs. A bun and the
  // apple tops are headgear — worn on the crown like a hat, never with one
  hairFront: ["none", "hairline", "blunt", "swept", "curtain", "sideLock", "cap", "mohawk", "tuft", "wisp", "curly", "helmet", "cloud"],
  ...
  // bean a crumpled ellipse · box · dress a bottom-heavy trapezoid · tube a narrow tube · vee an inverted trapezoid (shoulders wide, waist narrow — the bruiser's chest)
  body: ["bean", "box", "dress", "tube", "vee"],
  // Markings — three, and all of them lines (stripes · patch hatching · hatch), laid over the material. The imps' alone. Docs: character/parts.md § pattern
  // (dots — four square dabs on the body — and spots — three outlined dalmatian spots — were dropped)
  pattern: ["none", "stripes", "patch", "hatch"],
  ...
  material: ["graphite", "ink", "oil", "charcoal", "watercolour", "marker"],
  // The **body's** goofy material — `same` for one tool over the whole creature, or one of the six for a body made of something else
  ...
  bodyMaterial: ["same", "graphite", "ink", "oil", "charcoal", "watercolour", "marker"],
...
export const TOP_KNOTS = ["bun", "apple", "appleBig"];
...
export const LATE_SLOTS = ["legLength", "build", "tailSkin", "tailLength", "mouthPos", "mouthSize", "material", "density", "bodyMaterial", "tailDeco", "ghost", "bodyDensity", "browLength", "eyeScale", "hairBack", "pupil", "eyewearSize"];
...
export const DEFAULT_BIAS = {
  ...
  pattern: [["none", 4], ["stripes", 2], ["hatch", 2], ["patch", 1]],
  ...
  material: [["graphite", 1.5], ["charcoal", 1], ["oil", 1], ["ink", 0.8], ["watercolour", 0.9], ["marker", 0.9]],
  // Most creatures are one tool through; a body of its own is seasoning on top of seasoning
  bodyMaterial: [["same", 9], ["graphite", 1], ["charcoal", 0.7], ["oil", 0.7], ["ink", 0.5], ["watercolour", 0.6], ["marker", 0.6]],
