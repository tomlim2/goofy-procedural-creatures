...
**두피 위**(crown 2.06 — 머리 잉크 위·얼굴 아래, 뿔·모자와 같은 그룹) · **앞머리**(front 6.55 — 얼굴 위에 그린다; 눈썹·입 6.6은 앞머리 위에 그려진다).
뒷머리와 앞머리는 같은 그룹(`hairGroup`)이라 얼굴 돌림에 **같은 비율·같은 방향으로 아주 조금만**(12%/8%) 따라간다 — 머리에 붙은 것이고, 앞뒤 머리채가
갈라지면 안 된다. 그래서 긴 머리·트윈테일·포니테일(뒤)과 앞머리·옆머리 커튼(앞)이 된다.
