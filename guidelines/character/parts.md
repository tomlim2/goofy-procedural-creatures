...
`src/character/vocabulary/slots.js` `SLOTS`의 전체 목록. 19슬롯 131파츠. 그리기는 `src/character/draw/` (섹션 = 파일: `head.js` `face.js` `body.js` `limbs.js`).
...
☆_☆ 별눈·♥_♥ 하트눈은 눈 **종류가 아니다** — 놀람의 변형이다(감탄·반함). 놀람 이벤트가 star/heart 변형이면 4초 동안 눈 위에 덮개+글리프가
얹힌다 ([../motion/catalog.md](../motion/catalog.md) § 놀람, `scene/rig.js eyeFx`).
| sleepy | 아래로 감은 호 | ✗ |
