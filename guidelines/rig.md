...
- **limb pivot** — the shoulder (`shoulderY` in `draw/layout.js`: 22% of the body's height below bodyTop, on the torso's left/right outline — half-width per form: box 0.98 · bean 0.85 · dress 0.76 · tube 0.63; the head's chin never goes below this line, [character/parts.md](character/parts.md) § head) / the hip (0.02 above the hem) / a quad's root (25% of bodyH up). A limb is baked hanging from the pivot's origin. Arms are stood up with `bindArm(side)` (the T-pose) and the clock's `state.arms` supplies the joint angles
- **elbow** — the end of the upper bone. The lower bone is baked hanging from its origin. Upper:lower arm = 0.48:0.52; a biped leg's **knee** sits at 52% (thigh:shin = 0.52:0.48), the foot on its own **ankle** pivot at the shin's end. `armRig(spec)` passes the arm dimensions to the clock so it can solve actions by IK; the clock works a crouch's descent out of the leg split (motion/index.js)
