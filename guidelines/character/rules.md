...
- **두 눈은 조금만 겹치고, 겹치면 큰 눈이 앞이다.** `eyeGeometry`가 중심 거리를 반지름 합의 70% 이상으로 벌리고(자리가 없으면 두 눈을
  ...
  교차하는 윤곽선이 안 남는다. 정지 눈(hollow)은 채움+윤곽을 한 스케치에 작은 눈부터 그려 같은 효과. `census --check`가 70% 넘는 겹침을 잡는다
- **선으로만 그리는 눈은 좌우 대칭.** sleepy·line·happy·squeeze·droop·cross·half·side는 `eyeGeometry`가 크기·높이 어긋남(`eyeSizeSkew`·
  `eyeHeightSkew`)을 0으로 둔다 — 획 하나짜리 눈이 한쪽만 작거나 높으면 "작은 눈"이 아니라 실수로 읽힌다. 흰자·동공이 있는 눈만 짝눈을 허용한다
  (`layout.js LINE_EYES`)
