...
`src/vocabulary.js` `SLOTS`의 전체 목록. 17슬롯 102파츠. 그리기는 `src/draw.js`.

**규칙**: 슬롯은 **형태(생김새)** 만 담는다. 자세·동작은 `clocks.js` 상태다 ([parts.md](parts.md) 참조).
...
팔마다 **front(몸 앞)·back(뒷짐, 팔꿈치만 삐죽)** 두 벌을 굽는다. 어깨는 몸 폭 78%(윤곽 안쪽).
