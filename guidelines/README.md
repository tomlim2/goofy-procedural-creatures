...
| --- | --- | --- | --- |
| **Character** | `src/character/` | [character/types.md](character/types.md) species, archetypes, proportions, palette, constraints<br>[character/parts.md](character/parts.md) 34 slots, 236 parts | [character/rules.md](character/rules.md) the procedure for adding a part, separating form from motion, distribution standards |
...
| --- | --- |
| `/` · `index.html` **the name screen** | The front door ([name.md](name.md)). A name, a species (ANY lets the name pick) and DRAW turn a card that lies face down over onto the creature that name makes up — the name, the species and the release on it, nothing that ranks a name — and SAVE keeps the card as it is seen, and LINK hands its address on — in English or Korean, by the device's language. The same name draws the same card within a version of the code, and the card drawn rides in the address (`?name=…&species=…`), so a link shares it |
...
| The **name screen's** mapping and card | `node scripts/names.mjs` — the species on ANY at one in five, a name's variants as one name, an address bringing its card back, a Korean name for every species | [name.md](name.md) § checks |
