...
| **pup** | quad | The head (fur) color (about 1/3 the black-ish FURS), the body the same or a close tone. No markings — the pattern slot is the imps' | Hanging ears (flap/long), a muzzle — the outlined pad it shares with the cat — plus a black nose (the nose slot decides the form), the mouth in the muzzle below the nose — w (omega), o (open), the tongue (on ^^ too), line, dot, smile; the tail skeleton flag/stubtail/ring × the skin thick/plume, patches, legs mostly stub (stick, float, boots) | The head roll at all times, sniffing dips, a held ^^ happy eye, tail flicks |
| **cat** | quad | The head (fur) color (about 1/3 the black-ish FURS), the body the same or a close tone. No markings — the pattern slot is the imps' | Triangular crown ears (pointy/fold), a muzzle — the dog's outlined pad, drawn over the eyes — with the pink cat nose and the mouth in it, whiskers (length per individual — the long ones poke outside the outline; in front of the eyes in every state), the ω, 3, meow and blep mouths (fangs and a hiss when angry), a vertical pupil (slit), the tail skeleton curl/longtail/hook/kink × the skin thick/plume, legs stub·stick (float, boots) | The tail swish at all times, winks, big head tilts, stretches |
...
| --- | --- | --- | --- |
| **beast** | 3 | Horns, ears, teeth | horns(curved/straight), ears(pointy/flap), mouth(grimace/grin), nose(wedge/hook/broad), hair(spikes/hedgehog/mop), head(round/wide/jar) |
| **scholar** | 2 | Glasses, a bob, a beret | eyewear(glasses/monocle), eyes(dot/half), pupil(sleepy), hair(bob/helmet/bangs/longbob/wisp/curly/cloud/sweep), headgear(beret/fedora), mouth(line), nose(long/hook), horns(none) |
| **trooper** | 3 | A helmet, an eyepatch, stripes, boots | headgear(helmet/cap/band/pot), eyewear(patch/goggles), head(square/block/wedge), body(vee/box), hair(scribble/spikes/hedgehog), pattern(stripes/patch/hatch), arms(sleeve/stick), legs(boots) |
| **sprite** | 3 | Antennae, big eyes, long limbs | horns(antenna), eyes(wide/ring/spiral), head(tall/egg/wedge), body(tube), build(narrow/skinny), legs(stick/tiptoe), arms(stick/mitten), hair(none/wisp/tuft/pigtails), nose(none/dot) |
| **blob** | 2 | Wide, bald, blunt limbs | head(wide/round/pear/jar), hair(none/tuft/mop), eyes(dot/ring/half), body(bean/dress), build(wide), legs(stub), arms(stubby), horns(none/nub) |
| **wanderer** | 2 | A band, sleepy eyes, hatching | headgear(band/pot/cap/fedora), hair(scribble/mop/curly/bun), eyes(half), pupil(sleepy/cross), pattern(hatch/stripes/patch), mouth(wave/line), body(dress/bean) |
...
| | Dogs and cats: 50% exactly the head color / 30% the palette colour nearest a slightly darker tone (×0.9) / 20% nearest a slightly lighter one (×1.06). **Every colour a creature carries is a `PALETTE` entry** (palette.js): a derived tone is snapped to the nearest entry (`toned` / `nearestOf` in spec.js), never kept as a hex of its own, so it always answers to one of the editor's swatches | Being fur, the body has to be the same or close to the head to read as one body |
| | Imps: 50% exactly the head color / 30% the palette colour nearest a lighter tone (×1.35) / 20% nearest a darker one (×0.75) | Being a mass, the same. Settled after a color accent has landed on the head, so the body follows |
| ink | 1 of the 4 INKS | All dark brown-black. Imps are pinned to `IMP_INK` #1c1917 (darker than the head — in `PALETTE`, not in the INKS the roll deals) |
| **DARKS** | 9 colors | The dark palette for imp heads only. Ink · brown-black · brown-grey · light brown-grey · grey-blue · blue-grey · grey · purple-black · green-black. The body only shifts tone from here, snapped to the nearest palette colour (`toned`) |
...
colour that comes with it. **Colour belongs to the material** (`paintOf`, and `markInkOf` — which `paintPart` calls rather than repeating):
