...
### eyewear (5)
none / glasses (two circles plus arms, the lens radius = the eye × 1.45) / goggles (big circles plus a strap round the head, × 1.75) / patch (a patch over one eye, 1.5r × 1.35r, plus its strap) / monocle (one big circle plus a cord).
...
An eyepatch is always a **black** fill (an object) — on an imp's ink-black head it gets a light rim and only the strap is light ink. **The strap is the band round the head, tilted** — lowest at the patch and rising both ways from it, as a cartoon pirate's does: two lines leave the patch's upper corners and **stop on the head outline** (`layout.js` `headEdgeAlong` — the edge at that height whatever the head's shape, so a round head's strap does not stick out past it and a wide head's is not short of it), the long one up across the forehead at 26° to the far side, the short one out at 40° to the near side (`draw/face.js` `PATCH_STRAP`). Bangs and a hat brim are drawn above the eyewear, so they cover the far end as they would a real strap. It used to be one line to a fixed point at 0.45 of the head's half-height — level with a medium eye and downhill from a large one, grazing the other eye and running alongside its brow like a second brow. **Dropped when the eyes overlap** (when the patch radius of 1.5r
...
rings (back); the hedgehog, the cloud and the hood stay under a band only; under a cap the tufts and wisps go as well — its dome
covers the whole crown they root on, where the small hats and the band leave room round them · a mohawk wears nothing on the crown (a bun or an apple
...
one in three with the one slot) and about one in sixteen wears all three. Imps are bald outright — no fringe and nothing behind (`species.js` forbid and identity): the spiked ring one imp in four wore was the species' one hair, and a 도깨비 has horns, not hair.
...
### headgear (15) — humans, dogs, cats and the rex; imps never (species bias). Three of them are hair (below)
...
the skull); `pot` is the fall-through for a value the table does not know. none / helmet (a dome from above the brows to over the crown plus a rim and a ridge) / cap (a dome over the crown plus a brim to one side. **A covering hat worn over hair**, as the fedora is — a helmet and a pot take the hair off (`applyLateConstraints`), a cap keeps every fringe — so its dome is cut from **the hair's own dome** (`hair.js hairDome`: the head's drawn path with the scalp's volume on it), 1.06× it and 0.06 clear of the hair at the crown (`CAP_DOME`). Cut from the head's shape at 1.04×, as it was, it sat *inside* the hair: the head's path lumps with the creature's own noise and the scalp's puff multiplies those lumps, while the hat's own lumps fell elsewhere, so the hair came out over the dome's edge — a rim of hair riding above the cap, and a cap that read as sunk into the head rather than worn on it. The hair that is not a dome cannot be cleared that way — the tufts and wisps are spikes on the crown and stood up through the cloth — so a cap takes them off (`applyLateConstraints`, § hair). Its brim reaches where it did: 1.34× the dome's half-width (`CAP_BRIM`), where 1.5× the old, smaller dome put it) / band (a forehead band) / pot (a tub rising from above the brows to higher than the crown) /
beret (a tilted disc plus a nub) /
fedora (중절모 — a felt hat seen from the front, asked for by the owner, 2026-09-21: a tub of a crown **pinched in at the top** (a dent 0.16 of its height, its top 0.78 of its foot, rising 0.24·ry over the head's top — under the cell ceiling), a **ribbon** round its foot in the felt's own colour taken darker (0.55, a thick pencil stroke — a band, not a line), and the **brim**, a flat lens 1.55× the crown's foot wide, never past ±0.46 of the cell, tipped 0.05 rad to one side, its lower edge on the brow line at the eyes. **Worn over hair, like the cap**: the crown's foot is measured off the hair's own dome (`coverHair`) and stands 1.04× outside it, so no hair comes out between the brim and the crown; fringes and a bob's hem come out from under the brim, and the tufts and wisps come off under it as under a cap (`applyLateConstraints`). The brim's far half passes behind the crown — the lens is filled whole and outlined along its near arc and beside the crown only, and the crown's foot is bowed down over it. `FEDORA` in `headgear.js`. A scholar's and a wanderer's hat (1 · 1.5), 1 by default, the odd one on a dog or a cat (0.5) and a rex (0.4): 6% of humans, 4% of dogs and cats, 2% of rexes) / bonnet (a thick band crossing from eye level on both sides over the crown — **disabled**: it reads as frilly and is in no bias; assets and gallery only) /
...
| cat | pointy (the default — sides a touch concave, a blunt round tip) · pointyMid (narrow and tall — opening further, +0.15 rad) · pointyBig (wide and low — a round tip and convex sides) · round · roundMid · roundBig (the rounded ear at the pointed family's three builds — a dome most of the ear's width on convex sides; the board never deals it, the editor draws it when asked) | **Filled triangular bumps at the two corners of the crown (~35°; 30°, slightly inside, on a square head)** — the base tucked inside the outline so they are one mass with the head, the outline the same weight and drawn twice, the base laid along the tangent at the attachment point and the ear's axis halfway between that point's normal and vertical plus a per-kind opening (a round head opens them out, a flat head stands them straight, with a slight left/right difference). **Its sides are sampled, not straight**: it is the one shape on a creature that cannot be a blobPath (its base has to sit on the head outline and its tip has to end where the axis says), and drawn as two straight runs with a single bend — 0.9px of bow at a board cell — it stood on a head of 4~7 lumps looking like a ruler's triangle. Each side is cut into 7, the bow runs a bit over 2× deeper, and a wobble off the creature's own `hand` rides on it; both are enveloped by sin(πk), which pins the base and the tip exactly where they were. **The tip is a dome** — `tip` in `CAT_EAR` is its radius, the apex at the ear's height, and each side runs to where a straight side would touch that circle (θ = π/2 − atan(w/h) off the axis), so side and dome join without a corner. A flat top two points wide was a needle on the narrow ears, and with the sides bowed inward (−0.26) the ear was a thorn; the pointed kinds bow −0.08 and −0.06 now. **The wobble is one soft bend a side, never a wave** (under half a wave, 3~7% of the half-width): it used to run up to two waves at up to 16%, and on seven sample points that was two or three kinks on every side — a crumpled ear. The two sides of one ear and the two ears still draw their own bend, so no two cats and no two sides repeat. The inner ear is **the same shape scaled in, on the same curve** — the outer sides' own bends scaled with the width, so it is the ear inset rather than a second shape crumpled on bends of its own — its base at the ear's base position (0.012 inside), its width 0.62× the ear, its tip 0.7× the height and its dome's radius 0.62× — the one path builder (`earPath`) draws both. Per individual (double line 45% · fill 30% · none 25%). The board deals no round, fold, flap, long or none (measured across 70 reference images; `species.js` forbid) — `round` is drawn all the same, because the editor draws what a hand picks and the one value named round used to fall back to the pointed ear |
...
toward the light side on black fur) · none 25%. A crease went — one straight stroke from the middle of the root to half the height, 15% of cats, in the outline's ink: it read as a stick
in the ear rather than a fold, and its share is none's now, so no other cat's ear changed. The inner **line** is a mark on fur and uses `spec.faceInk` —
