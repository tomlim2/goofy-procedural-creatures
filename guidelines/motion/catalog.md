...
| tailAngle / tailTip / tailPuff | rad / rad / 0~1 | tailGroup.rotation.z / tailTipGroup.rotation.z(끝 마디, 뿌리 기준) / tailGroup.scale(1 + 0.3·puff) |
...
| **얼굴 돌림** (좌우·위아래) | 시선 추종, 임계감쇠 w 0.1 (≈ 0.8초). 머리카락·모자·뿔(crownGroup)은 이목구비 이동량의 45%(x)·30%(y)만 같은 방향으로, 귀(earGroup)는 40%(x)·15%(y) **반대 방향**으로, 앞머리(bangsGroup)는 12%(x)·8%(y)만(거의 고정) 밀린다 — 위치만, 크기 그대로 | yaw 배율 human 0.5 / pup 0.7 / cat 0.8 / imp 0.6 (위아래는 ×0.6) | 이목구비 전체(눈·코·입·눈썹·안경·볼·수염·주둥이)가 머리 안에서 밀리고 살짝 눌린다. 동공이 먼저 가고 얼굴이 뒤따른다 |
...
| 윙크 | 8~20초, **3~5초** | **cat만** | 그쪽 눈만 ^^ (정지 눈도). 표정은 무조건 3초 이상 |
...
꼬리는 **뿌리 마디**(tailGroup, `tailAngle`)와 **끝 마디**(tailTipGroup — 척추 55% 지점 피벗, `tailTip` = 뿌리 기준 상대각)다.
`table.js` `tailTip`이 끝 마디 파라미터. 개와 고양이는 **반대**로 읽는다 — 개는 빠른 흔들기가 기쁨이고, 고양이는 빠른 움직임이 짜증·흥분,
...
| **세움** raise | 기분 좋음에 딸림 | — | ♥ 놀람 시작·♥ 이모지 때: 뿌리 +0.9rad(0.4초에 들어 3~5초 유지, 0.6초에 내림). 두 가지(반반) — **hook** 끝을 안으로 굽혀 세우고 끝 ±0.08 12Hz 떨림 / **straight** 빳빳이 곧게(+0.15 더, 끝은 굽음을 펴 −0.25, 떨림 없음) |
| **부풀림** puff | 놀람에 딸림 | — | 놀람 시작 1.2초 동안 꼬리 배율 1 → 1.3 (`tailPuff`) |
