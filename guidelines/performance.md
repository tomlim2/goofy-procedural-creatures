...
| | 재질 공유·메시 병합 전 | 지금 |
| --- | --- | --- |
| draw call / 프레임 | 1313 | **550** |
| 렌더 JS 시간 / 프레임 | 3.3 ms | **0.8 ms** |
| 재질 객체 | 1611 (메시마다 하나) | **4~6** (불투명도별 공유 + 이모지) |
| 삼각형 / 프레임 | 119k | 59k (한 번만 그린다) |
| 판 굽기 (`scene.build`) | 55 ms | 52 ms |

9×6(54마리)은 draw call 848, 프레임 1.3 ms. 굽기는 74 ms — NEW SEED가 즉시라 굽기 쪽은 더 줄일 이유가 없다.
...
굽는다 — 채색은 전부 불투명이라 같은 renderOrder에서 채색이 먼저 그려지면 그만이다. 예외는 얼굴(face)·정지 눈(staticEyeBack/Front) 층 —
정지 눈의 채움(동공·흰자)이 얼굴 잉크(수염) 밑, 정지 눈 잉크가 그 위에 와야 해서 채색 2.3·잉크 2.4를 따로 둔다.
눈 리그의 흰자·테도 한 메시다. 계층·번호는 [rig.md](rig.md) § 계층.

층을 나누는 건 **따로 끄고 켜야 할 때**뿐이다 — 정지 눈은 눈마다 한 층(`staticEyeBack`·`staticEyeFront`)인데, 윙크가 한쪽 눈만 아치로 바꾸려면
그 눈의 층만 꺼야 해서다(두 눈이 한 층이면 반대쪽 눈이 사라진다). 두피 위 머리카락도 뿔과 층이 다르다 — 얼굴 돌림의 깊이가 다르다([rig.md](rig.md) § fake 3D 깊이).
개체 하나가 보이는 메시는 15개 안팎이다(층 13 중 비어 있지 않은 것 + 팔다리·꼬리 마디·눈썹·입·눈).
층을 새로 만들 때는 "기존 층에 그려도 되나"를 먼저 묻는다 — 층 하나가 판 전체에 35~54 draw call이다.
