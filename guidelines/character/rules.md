...
- **A closed eye is still an eye — redraw it, do not cover it.** When an eye closes (blink, sleep, anger) the open eye is **switched off** and the shut line (`shut`) or fierce eye (`angry`) is drawn in its place. **^^ and a wink stay in the ball**: an eye with an eyeball keeps its white and rim and its pupil becomes the ^^ arch — the `happy` pupil's own mark (a live eye swaps the pupil mesh for the arch, a static eye that keeps a pupil under its lid is redrawn with `pupil: happy`, `scene/rig.js lidSketches`); only an eye that keeps no mark (dot, slit, hollow, the heavy-lidded three) becomes the arch itself.
