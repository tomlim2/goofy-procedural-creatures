// State sets for brows and mouth — brows rest / alt / angry, mouth rest / alt / ^^. The scene stands a mesh up per set and switches them by clock state
// (browAlt, angry, mouthAlt, happy). Anger is the brows' alone: the mouth keeps its own (the owner, 2026-09-16 — the bared-tooth angry mouth went with the fierce eye).
...
// Set list: brow [rest, alt, angry] · mouth [rest, alt, ^^]. Species forbid applies to the values — a species with no brows (dogs, cats)
...
export function facePartKinds(spec) {
  ...
  return {
    ...
    mouth: [mouth, allow("mouth", ALT_MOUTH[mouth] || "line"), allow("mouth", HAPPY_MOUTH[spec.species] || mouth)]
  };
}
