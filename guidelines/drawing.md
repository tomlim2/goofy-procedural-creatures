...
| line | `ink.line(path, { color, size, joint })` | an open line, **down to a dot or a dash** — a brow, a lid, a whisker, a limb, a strand, a horn, the floor, a tooth's edge, a claw, the dot mouth. `joint = [start, end]` marks an end that meets another line or a fill's edge (the tail's root, the tip's arc, a leg's knee): no overshoot, no thinning there, and the end lands **exactly on its point** at the plain width — the wander and the breathing ease in over `PENCIL.joint.fade` from it, so two bones meeting at a pivot are one line through it however far it folds (left loose, each end wandered up to 0.4 widths its own way and a leg had a step at the knee) | **S** the finest (hair strands, whiskers, a tail's hairs, the dots and dashes) · **M** the rest · **L** two lines only, an imp's horn and a stub tail |
...
same tint of the ground, so they come *up* out of the surface (`opened` in `materials.js`). How far the tint reaches depends on what it is laid on: on a **dark** ground
...
vanished into a black body, so a dark creature with a textured material read as a solid blob.
...
water (within 45 of luminance of the light ink) takes a lighter tint of itself for a scratch, a little more the more open the step.
Ink is the one technique this touches — it is the one whose marks go lighter on a light ground — and its `tone` in the table is a
scratch's own mild watering. A part names a
...
used to pull the base toward the technique's own tone as well (graphite and charcoal darkened it, ink lightened it, oil painted it
deeper, by the texture's `pull` × how far the step went) so that the five steps would survive the board — a 7×5 cell puts 144 device
pixels in a world unit, the fine marks fall under one there, and carried by marks alone the steps measured 0.7~4.4 of luminance
apart; but with the pull a creature's palette colour darkened as its density rose, and a skin at black was not the skin the palette
named. The pull is gone; the marks are drawn as before, the lights at their per-step widths and counts, over the colour itself, and
stay **as fine as the hand would draw them** (coarsening them until they could carry a value on their own turns a small part into
blotches and a face into camouflage). **The step is the creature's own**: the `density` slot holds one of the five outright and the roll picks it (`stepOf`), so every
