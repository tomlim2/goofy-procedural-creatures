...
`src/character/vocabulary/slots.js` `SLOTS`의 전체 목록. 23슬롯 170파츠. 그리기는 `src/character/draw/` (섹션 = 파일: `head.js` 윤곽·귀 · `hair.js` 머리카락 ·
...
| sharp | 날카로운 나뭇잎 눈 — 코 쪽 끝이 뾰족하게 내려가고 위 선이 곧고 굵다, 아래는 불룩. 바깥으로 치우친 동공. 늘 심술난 인상 | ✗ |
| cross | X | ✗ |
...
**상태 벌**(`faceStates.js`): 쉼 / 대체(`ALT_MOUTH` — dot↔line, line↔wave, smile→grin, omega↔three, grimace→line, tongue→open, fangs→line, shout→open, meow·blep→omega, zigzag·scribble→wave, frown·bracket→line, pout→dot) /
