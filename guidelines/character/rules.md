...
- A dot mouth, a dot nose and a vertical pupil are **marks**, drawn with `PENCIL_DAB` — a blunt stub that keeps its own length and does not shed ([../drawing.md](../drawing.md) § the outline). `Sketch.stroke` draws nothing shorter than its re-sample step: its taper takes both ends to nothing
