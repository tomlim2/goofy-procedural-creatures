...
with its name and its species. The same name always draws the same one.
...
three.js comes from unpkg via an importmap — the minified module build (`three.module.min.js`, 170 KB gzipped against the plain build's 264). The check scripts (§ Scripts) import `three` in node, so they need `npm install` once.
...
`/` — **the name screen**, the front door ([guidelines/name.md](guidelines/name.md)). The card, and under it a name field with a species
(ANY lets the name pick) and DRAW — nothing else, no header and no nav — in English, or in Korean when the device speaks it
(`src/lang.js`; `?lang=ko` asks for it). The card lies face down (MENAGERIE · TYPE A NAME, in the goofy letters — the name written in its place as it is
typed) until a DRAW turns it over; the field keeps the keyboard, a click on the card leaving the caret in it. The name is the seed: it becomes a roll and a species, and the creature stands up on a
...
DANCE (`D`) puts every biped into the chorus of *Dumb Ways to Die* as the video dances it, all on one beat: most hula and pace, spin, jump
and then sway with their arms up; some stand and clap over their heads; some only sway; everyone sings. The quads stand through it
([guidelines/motion/catalog.md](guidelines/motion/catalog.md) § the dance).
...
(how hair is grown — fur balls), the goofy type (any script traced off Noto Sans — what the name screen's card is written in) and the goofy letters (hand-lettered capitals as strokes in the pencil), then the shapes, the colors and the boil. Every figure is drawn live by the same `src/stroke.js` and `src/medium/` that draw the creatures, at the board's own scale, and the
...
| ACTION AUTO / IDLE / SLEEP / SIT / WALK / DANCE / an action | Forces one action (that layer only; the others idle) — arms (waving, arms up, arms crossed, a salute…, bipeds), body (hopping in place, everyone), quad (scratching, wagging). SLEEP lies quads down to sleep, SIT sits them (quad actions still follow their schedule — they scratch and wag while sitting), WALK walks everyone — out from home and back (arm actions still follow their schedule), and DANCE puts every biped into the Dumb Ways to Die chorus on one beat — hula, spin, jump, arms up and swaying, claps for some (the quads stand through it; the arms are the dance's). AUTO lets the layers overlap, with dogs and cats sleeping and sitting now and then and everyone walking now and then. IDLE is every layer idle and awake |
...
| `index.html` · `src/name.js` · `src/card.js` · `src/character/name.js` | The name screen, the front door. `character/name.js` turns a name into a key, a roll and a species (the one door into the generator that takes something from a visitor); `card.js` is the card — where everything stands, its three words laid out in the goofy type (`medium/type.js`, traced off the fonts in `fonts/`); `name.js` is the page — the scene at 1×1 with the card's frame drawn in the pencil, and SAVE | [name](guidelines/name.md) |
...
| `fonts/` | The name screen's type — Noto Sans and Noto Sans KR at 500 and 700, subset to 1.6 MB, under the SIL Open Font License; loaded whatever is typed | [README](fonts/README.md) · [name](guidelines/name.md) § the type |
...
node scripts/names.mjs                 # the name screen on its own — the species on ANY, a name's variants as one name, an address bringing its card back, a Korean name for every species
...
34 slots, 237 parts. A slot holds form only (the `material` slot a surface's goofy material — its look); pose and action are motion. Length and build (`armLength`, `legLength`, `build`) are dimension
