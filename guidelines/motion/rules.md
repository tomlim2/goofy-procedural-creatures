...
**캐릭터에는 자세가 없다.** 모션이 없을 때 캐릭터는 바인드 포즈(T포즈)다. 모션은 **행위**다 —
만세, 손 흔들어 인사, 팔짱, 팔을 파닥이며 좋아하기, 그리고 아무 행위도 없는 idle(호흡·지터·보일).
행위는 바인드에서 시작해 바인드로 돌아온다. "이 개체는 뒷짐진 채 쉰다" 같은 개체별 쉼 자세는
없다 — 그건 캐릭터에 모션을 섞은 것이다.

## 행위는 온몸이다 — 두 팔을 다 정한다

"팔을 흔든다"는 행위가 아니다. "팔을 흔들어 인사한다"가 행위다 — 한 팔은 들어 흔들고 **다른 팔은 내린다.**
행위(`actions.js` `ACTIONS`)는 항상 `[활동 팔 자세, 나머지 팔 자세]` 둘을 적는다. 한 팔만 정하고 나머지를
바인드(T)에 남기면 마네킹이 한 팔만 까딱이는 그림이 된다. 옛 "팔 들기"·"손 흔들기" 이벤트가 그랬고, 그래서 없앴다.
팔다리 하나에 붙는 오프셋 이벤트를 새로 만들지 않는다 — 행위를 만든다.
...
`index.js`의 init 25단계와 update 순서는 고정이다. 새 모션은 **각 블록의 끝에** 붙인다.
