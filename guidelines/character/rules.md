...
얼굴은 움직인다 — 놀라면 흰자가 1.65배, 잠들면 눈꺼풀이 덮이고, 입·눈썹은 대체 벌로 바뀌고, 8방향으로 돌아간다. 파츠가 "그려져
...
- 코·입·볼은 **눈이 최대로 커졌을 때의 밑선 아래**에 앉는다 (`eyeFloor` — 왕눈·외눈처럼 눈이 그 x에 닿을 때만). 놀람 배율은
  `SURPRISE_REACH`(1.65) × `eyePop`(wide·cyclops 0.5) — 왕눈은 반만 커진다 (`scene/rig.js eyePop`, `animate.js`)
