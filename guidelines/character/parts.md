...
fold **접힌 귀**(버튼 이어) — 넓은 밑동이 서고 그 위 **덮개 세모가 바깥·아래로 늘어져** 밑동 윗부분을 덮는다. 끝이 **접힘선보다 아래**로 내려오고
밑동 밖으로 조금 나가야 접힌 귀다(위로 굽으면 뿔, 옆으로만 뻗으면 깃발). 덮개는 귀 안쪽 면이라 조금 더 어둡고, 접힘선은 **잉크 한 획** — 검은 털에서는
두 조각의 색이 같아 선이 없으면 접힌 게 안 보인다 · none 없음. 종족 bias flap 4 · long 3 · pointy 2 · round 1.5 · fold 1.
