...
└── headGroup                ← 원점 = 목(neckY = bodyTop). 갸웃·롤·끄덕·딥. 윤곽만 여기 직접
    ...
    ├── earGroup             ← 귀·뒷머리. 얼굴 돌림 때 이목구비 이동량의 [−0.4, −0.15]배 — **반대 방향**으로 밀린다(크기 그대로)
    │   ├── hairBackFrame ×3     ← 뒷머리(긴 머리·트윈테일·포니테일·큰 덩어리의 바깥). 머리·귀 뒤, 몸 위 (1.55)
    │   ├── crownBackFrame ×3    ← 옆귀(사람·도깨비). 머리 채색 뒤 (1.7) — 뿌리가 머리에 가린다
    │   └── frontFrame ×3        ← 개 귀·고양이 귀. 머리 잉크 위 (2.12) — 윤곽·머리카락 밑동을 덮되 눈은 못 덮는다
    ├── crownGroup           ← 뿔·머리카락·모자. 얼굴 돌림 때 [0.45, 0.3]배 — 같은 방향으로 덜 밀린다(크기 그대로)
    │   ├── crownFrame ×3        ← 뿔·머리카락. 머리 잉크 위 (2.06)
    │   └── hatFrame ×3          ← 모자. 귀 위·얼굴 아래 (2.16) — 귀 밑동·머리카락을 덮는다
    ├── bangsGroup           ← 앞머리. 얼굴 돌림 때 [0.12, 0.08]배 — 얼굴 위에 그리지만 머리에 붙은 것이라 **아주 조금만** 밀린다
    │   └── hairFrontFrame ×3    ← 앞머리 띠·옆머리 커튼·두건형 앞. 얼굴 위(6.55), 눈썹·입(6.6) 아래
    └── faceGroup            ← 원점 = 머리 중심(headCy). 얼굴 돌림으로 x/y 이동 + 눌림. 이목구비 전부
        ├── faceFrame ×3         ← 보일 변형. 볼·수염 — 채색(2.3)·잉크(2.4) **두 메시** (정지 눈과 엇갈려야 해서)
        ├── staticEyesFrame ×3   ← 보일 변형. 정지 눈(dot·half·slit…)만 따로 — 채색(2.3)·잉크(2.4) 두 메시. 놀람 변형(☆·♥) 때 끈다
        ...
        ├── faceStates.mouth ×2
        ├── staticLid ×(정지 눈 수) ← 정지 눈(dot·cross·slit…)의 감은 눈 선·미소 아치(3.6). 잠(sleep > 0.5)이면 감은 선, ^^·윙크(그쪽)면 미소 아치 — 그때 정지 눈 프레임을 끄고 대신 선다 (덮개 없음)
        └── eyeRig ×(0~2)        ← 살아 있는 눈만
            ├── open{흰자+테 한 메시 · pupil} · smile · shut — 눈 블록 o(뒷눈 3 · 앞눈 3.5)부터 +0 · +0.2 / +0.35 / +0.35. 감을 때 open을 **끄고** shut(감은 선, 눈꺼풀 > 0.5)이나 smile(^^·윙크)이 대신 선다 — 살색 덮개는 없다
...
| 1 | 바닥선 |
| 1.5 | 몸 (채색+잉크) |
| 1.55 | 뒷머리 — 머리·귀 뒤, 몸 위 (earGroup). 실루엣 밖으로 나온 부분만 보인다 |
| 1.7 | 옆귀 (사람·도깨비) — 머리 채색 뒤라 뿌리가 머리에 가린다 (earGroup) |
| 2 | 머리 (채색+윤곽 잉크) — 채색이 **몸 잉크 위, 불투명**. 머리가 몸통을 덮는 자리에 몸통 윤곽선이 비치지 않게 |
| 2.06 | 뿔·머리카락(두피 위) — 윤곽 위 (crownGroup) |
| 2.12 | 개 귀·고양이 귀 — 채색 불투명 (윤곽·머리카락 밑동을 덮는다 — 귀가 실루엣의 혹으로 붙는다) (earGroup) |
| 2.16 | 모자 — 귀 위, 얼굴 아래 (crownGroup) |
| 2.3 | 얼굴 채색 (볼) · 정지 눈 채색 (동공·흰자) — 모자 위 (눈이 모자에 가리지 않는다) |
| 2.4 | 얼굴 잉크 (수염·주근깨·눈물) · 정지 눈 잉크 |
...
| 6.55 | 앞머리 — 코·안경 위, 눈썹·입 아래 (bangsGroup) |
...
| --- | --- |
| 층 11개(몸·뒷머리·옆귀·머리·뿔/머리카락·개/고양이 귀·모자·얼굴·정지 눈·얼굴 앞·앞머리) × 보일 3벌 — 층당 메시 하나 | visible 토글 |
