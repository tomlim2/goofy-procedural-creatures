...
`src/character/vocabulary/slots.js` `SLOTS`의 전체 목록. 21슬롯 145파츠. 그리기는 `src/character/draw/` (섹션 = 파일: `head.js` `face.js` `body.js` `limbs.js`).
...
| sparkle | ◕ 반짝 왕눈 — 흰자 + 흰자를 거의 채우는 동공(0.7r) + 하이라이트. 시선 0.14r | ● |
...
| --- | --- |
| none | |
| bob / mop / scribble / sweep | 두피를 덮는 **스크리블** (`Sketch.scribble`) — 레퍼런스처럼 부피가 있다: 호가 귀 높이(depth 0.56~0.62, sweep 0.4)까지 내려오고 넓게 퍼진다. 옆으로 내려간 끝은 귀를 덮지 눈까지 오지 않는다 |
| helmet | 두건형(바가지·투구) — 머리보다 살짝 큰 덩어리를 정수리부터 앞은 **눈썹 선**, 옆은 귀 아래까지. 촘촘한 세로 획(직모) + 위 경계 선 + 눈썹 선 톱니 끝단. 눈은 못 덮는다 |
| cloud | 구름형 곱슬 — 스캘럽(부채꼴) 윤곽의 큰 덩어리(머리 ×1.2) + 안은 고리 스크리블 + 가장자리 작은 고리들. 밴드와 같이 나온다 |
...
사람만 머리카락이 있다(개·고양이는 forbid로 전부 none, 도깨비는 spikes/none). 앞머리·옆단발·두건형은 모자·밴드 밑으로 나와도 되는 "짧은 머리"에
들고, 구름형·고슴도치는 밴드와만 같이 나온다. 눈을 덮는 머리는 없다 — 앞은 눈썹 선이 하한, 옆은 귀까지 ([rules.md](rules.md)).
