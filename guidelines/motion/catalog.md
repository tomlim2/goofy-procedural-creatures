...
| eyeFx | { kind: "star"·"heart", k } or null | The startle variant — the pupil becomes a ☆/♥ in the ball (a live eye swaps its pupil mesh for the glyph, a static eye with a white is redrawn with it, a heavy lid lifts; the bare dot eye becomes the glyph itself — rig.js lidSketches), popping in and out by k |
| happy | bool | The pupil becomes a ^^ arch inside the eye (an eye with no ball closes into the arch itself) |
...
| Startle (the pupil shrinking) | Interval per the table, 4.0 s | human 8~22 / pup 10~26 / cat 9~24 / **imp 6~14** (with a 4 s hold, the minimum start interval is 6 s — so they do not run together) | The eye size is unchanged and **only the pupil goes to 0.5×** (on a live eye). **It shrinks in 0.1 s, holds 3.8 s and releases in 0.1 s** (`envelope`). **Variants** — drawn at the start, plain 60% / star 25% / heart 15%: with star or heart the pupils **become** ☆ or ♥ in the eye (the star outlined in the ink in the white, the heart in its red; a heavy lid lifts for it; only the bare dot eye becomes the glyph itself — `state.eyeFx`), and heart fires the ♥ emoji with it |
| ^^ happy hold | 6~16 s, **3~6 s** | **pup only** | An eye with an eyeball keeps its white and its pupil becomes the ^^ arch (a live eye swaps the pupil mesh, a static eye with a white is redrawn with a ^^ pupil); every eye with a white keeps it — the heavy-lidded set lifts its lid for it and the hollow eye's empty white takes it; only the bare dot eye becomes the arch itself. **A ♥ emoji makes it ^^ for 3.2 s** (every species, whatever fired the ♥) — and a dog follows it with a wag |
| Wink | 8~20 s, **3~5 s** | **cat only** | Only that eye becomes ^^ — its pupil the arch (static eyes too — only that eye's layer is switched off and its smile stands in, the other eye stays). An expression is always 3 s or more |
| **Anger** | 25~60 s, **3~5 s** | **cat and rex** (`angry` and `angryHold` in the table; null on the other species) | **The brows alone**: a species with brows gets the angry brow — the third brow set — and a face with no brows shows nothing. The fierce eye (＼ ／ over a glaring dot) and the bared-tooth angry mouth went on 2026-09-16: the owner wanted the eyes left alone by every mood. The envelope follows the same law as the startle eye: **0.1 s hard up / hold / 0.1 s release** (`state.angry` 0~1). It beats ^^, a wink and the ☆♥ variants (none of them shows while angry), and sits below sleep. **This is when a cat's tail fur bristles** (§ the tail, bristle) |
...
| Mouth switch | 4~12 s, 0.8~2.2 s | shared | The ALT_MOUTH table — a slight shift to a neighbour in the same mood (line↔wave, dot↔3, smile→grin, grid→line…; [../character/parts.md](../character/parts.md) § mouth). There are three mouth sets: rest, alt and **^^** (the panting tongue on dogs only). Priority ^^ > alt > rest |
...
armed biped), a cat-cadence **anger** (every 22~55 s for 3~5 s — the angry brows, measured at 7.1% of ticks)
and emojis bang · quest · heart · dots.
