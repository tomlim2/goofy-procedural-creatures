...
export const NO_MARK_EYES = ["dot", "hollow", "slit", "lidded", "sharp", "soft"];
export function applyLateConstraints(parts) {
  // **A mark needs an eyeball, in the open.** The pupil slot's marks (the little closed eyes, an X, the bracket, a spiral,
  // a scrawl) sit in a white; an eye with no ball (a bare dot), the hollow eye (no pupil by definition) and the slit (its
  // own pupil) have nowhere to put one, and the heavy-lidded three (lidded, sharp, soft) keep their round pupil — under
  // that thick lid a mark is a smudge. Their pupil is the dot — the one mark that stands on its own. A fixed overwrite,
  // no rng. Here and not in applyConstraints: the pupil is a late slot, not rolled yet when that runs
...
export function deriveSpec(next) {
  ...
  if (parts.ghost !== "none") { parts.eyes = "hollow"; parts.pupil = "dot"; }   // a ghost has empty eyes — nothing is looking back, and no mark sits in an empty eye
