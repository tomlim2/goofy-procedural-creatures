...
- **선으로만 그리는 눈은 좌우 대칭.** sleepy·line·happy·squeeze·droop·cross·half·side는 `eyeGeometry`가 크기·높이 어긋남(`eyeSizeSkew`·
  ...
  (`layout.js LINE_EYES`)
- **얼굴 잉크는 머리색 휘도로 정한다.** 머리색 휘도 < 120(도깨비 먹빛, 색 포인트가 피부에 붙은 파랑·초록·붉은 갈색)이면 이목구비를 검정
  대신 밝은 잉크(`faceInk` #e9e3d5)로 그린다 — 짙은 색 위 검정 선은 대조가 없어 눈·입이 안 읽힌다. 개 입은 밝은 주둥이 위라 늘 검정
- **표시는 얼굴 잉크, 물건은 제 색.** 도깨비의 밝은 `faceInk`는 선·점(눈·입·눈썹·수염 같은 표시)에만 쓴다. 안대·모자·안경알처럼
  ...
  실수처럼 보인다 (안대가 그랬다)
- **감긴 눈도 눈이다 — 덮지 말고 바꿔 그린다.** 눈을 감을 때(깜빡임·잠·^^·윙크) 뜬 눈을 **끄고** 감은 눈 선(`shut`)이나 미소 아치(`smile`)를
  그 자리에 그린다. 살색 덮개로 가리지 않는다(덮개는 안대·상처처럼 읽힌다). 반쯤 감긴 중간 단계는 없다 — 뜨거나 감거나. 정지 눈도 같다
  (`staticLids`, 정지 눈 프레임을 끈다). 조사도 "동공 안 보여도 됨"이 아니라 "그 자리에 감은 눈 선(또는 미소 아치)이 보여야 함"으로 센다
