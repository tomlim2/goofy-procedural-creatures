...
**안쪽 귀** — 귀 모양을 0.58배로 줄여 위로 조금 밀어 넣은 얼룩(윤곽선 없음). 개체마다 같은 계열 톤 45% · 분홍(코·볼터치와 같은 색) 30% · 없음 25%
(`wobbleSeed`, rng 없음). 늘어진 귀(flap·long)와 접힌 쪽 덮개에는 안 그린다 — 그 자세에서는 귀 **바깥면**이 보인다.
