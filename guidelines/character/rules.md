...
3. **`src/character/draw/`** — add the branch in the file the part belongs to: outline and ears `head.js` · hair `hair.js` (one function in the `HAIR` table) ·
   ...
   A mouth also needs a place in the alt and ^^ tables in `faceStates.js`
...
- **A closed eye is still an eye — redraw it, do not cover it.** When an eye closes (blink, sleep) the open eye is **switched off** and the shut line (`shut`) is drawn in its place. **^^, a wink and a startle's ☆·♥ stay in the ball**: an eye with an eyeball keeps its white and rim and its pupil becomes the mark — the `happy`, `star` and `heart` pupils' own drawings (a live eye swaps the pupil mesh for the mark; a static eye with a white — slit, side, half, hollow, the heavy-lidded three (`EYEBALL_KINDS`) — is redrawn with the state's pupil forced to the mark (`drawEyes` `state.pupil`, `scene/rig.js lidSketches`), so a cat's almond stays and its spindle becomes the mark, **a heavy lid lifts** and the mark stands in the open white, and an empty eye takes the mark in its white). **A white is never taken away by a smile or a startle**; only the bare dot eye becomes the mark itself. **Anger touches the brows alone** — the eyes and the mouth stay as they are (the fierce eye and the bared-tooth angry mouth went, 2026-09-16).
