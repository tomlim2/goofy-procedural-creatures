...
| 2.12 | Dog and cat ears — the fill opaque (it covers the outline and the hair's root, so the ear attaches as a bump in the silhouette) (depth 0.2). **With any headgear the layer moves to 6.59, above the hat** — the cartoon way, the ears poke through the cap; at 2.12 a cap swallowed a cat's crown ears whole. Bare-headed it stays at 2.12, under the face, which is what guarantees the ears cannot cover the eyes — hatted, the audit holds that guarantee |
