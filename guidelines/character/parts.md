...
### headgear (9) — humans, dogs and cats; imps never (species bias)
...
halo (a thin ink ring floating clear above the head — ink only, a mark rather than a thing with a colour; **the one headgear that keeps every hairstyle**, `applyConstraints` exempts it because it covers nothing). The color is accent or pop.
...
half-width at that height, fitting the head's size and shape. A hat is a separate layer **in front of** the head — it covers the outline, hair, horns and a human's or an imp's side-ear roots but cannot cover the eyes or eyewear.
A dog's and a cat's crown ears are the exception: with any headgear their layer rides **above the hat**, so the ears poke through the cap instead of vanishing under it ([../rig.md](../rig.md)).
