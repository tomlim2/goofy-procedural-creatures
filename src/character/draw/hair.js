...
//   back  뒷머리 — 머리·얼굴 **뒤**(1.55, 머리카락 그룹). 머리 실루엣 밖·어깨 위로 보이는 부분만 남는다 (긴 머리·트윈테일·포니테일·큰 덩어리)
//   crown 두피 위 — 머리 잉크 위·얼굴 아래(2.06, 뿔·모자와 같은 crown 그룹). 정수리 캡·가시·똥머리·사과머리
//   front 앞머리 — 얼굴 **위**(6.55, 머리카락 그룹). 앞머리·옆머리 커튼. 눈썹(6.6)은 앞머리 위에 그려진다
// back·front는 같은 그룹(hairGroup)이라 얼굴 돌림에 **같은 비율·같은 방향으로 아주 조금만**(12%/8%) 따라간다 — 앞뒤 머리채가 갈라지지 않는다
