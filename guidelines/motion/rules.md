...
The same holds for a **profile** — a whole way of moving that is not a species, like the ghost's ([catalog.md](catalog.md) § the ghost). Build it as a
function of the species' own table (`ghostMotion(M)`), so a ghost cat still moves like a cat, and take motions out by the table's own means: `null` means this
