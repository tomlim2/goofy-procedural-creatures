...
| `src/stroke.js` | The lines — strokes → ribbon geometry: `stroke()` (wobble, pressure, taper), `pencil()` (the reference's line, its numbers in `PENCIL`), the fill-up, scribbles, hatching; `buildGeometry` (several sketches → one geometry). A `Sketch` draws the three concepts by name — `contour()`, `paint()`, `fur()` — delegating to `medium/` | [drawing](guidelines/drawing.md) |
