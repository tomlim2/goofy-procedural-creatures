...
- **A closed eye is still an eye — redraw it, do not cover it.** When an eye closes (blink, sleep, anger) the open eye is **switched off** and the shut line (`shut`) or fierce eye (`angry`) is drawn in its place. **^^ and a wink stay in the ball**: an eye with an eyeball keeps its white and rim and its pupil becomes the ^^ arch — the `happy` pupil's own mark (a live eye swaps the pupil mesh for the arch; a static eye with a white — slit, side, half, hollow, the heavy-lidded three (`EYEBALL_KINDS`) — is redrawn with the state's pupil forced to the arch (`drawEyes` `state.pupil`, `scene/rig.js lidSketches`), so a cat's almond stays and its spindle becomes the arch, a heavy lid keeps its white with the ^^ under it, and an empty eye takes the arch in its white). **A white is never taken away by a smile**; only the bare dot eye becomes the arch itself.
