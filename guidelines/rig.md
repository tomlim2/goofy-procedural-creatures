...
└── headGroup                ← 원점 = 목(neckY = bodyTop). 갸웃·롤·끄덕·딥. 윤곽만 여기 직접
    ...
    ├── crownGroup           ← 뿔·두피 위 머리카락·모자. 얼굴 돌림 때 [0.45, 0.3]배 — 같은 방향으로 덜 밀린다(크기 그대로)
    │   ├── crownFrame ×3        ← 뿔·두피 위 머리카락(정수리 캡·가시·똥머리). 머리 잉크 위 (2.06)
    │   └── hatFrame ×3          ← 모자. 귀 위·얼굴 아래 (2.16) — 귀 밑동·머리카락을 덮는다
    ├── hairGroup            ← 뒷머리·앞머리. 얼굴 돌림 때 [0.12, 0.08]배 — 머리에 붙은 것이라 **아주 조금만**, 둘이 **같은 비율·같은 방향**으로 밀린다
    ...
    └── faceGroup            ← 원점 = 머리 중심(headCy). 얼굴 돌림으로 x/y 이동 + 눌림. 이목구비 전부
        ...
        ├── staticEyeBackFrame ×3 · staticEyeFrontFrame ×3  ← 보일 변형. 정지 눈(dot·half·slit…) **눈마다 한 층**(작은 눈 Back · 큰 눈 Front — 겹치면 큰 눈이 앞). 채색(2.3)·잉크(2.4) 두 메시.
        ...
        ├── faceStates.mouth ×2
        ├── staticLid ×(정지 눈 수) ← 정지 눈(dot·cross·slit…)의 감은 눈 선·미소 아치(3.6). 잠(sleep > 0.5)이면 감은 선, ^^·윙크(그쪽)면 미소 아치 — 그때 그 눈의 정지 눈 층(lid.frames)을 끄고 대신 선다 (덮개 없음)
        └── eyeRig ×(0~2)        ← 살아 있는 눈만
...
| 1 | 바닥선 |
| 1.5 | 몸 (채색+잉크) |
| 1.55 | 뒷머리 — 머리·귀 뒤, 몸 위 (hairGroup — 앞머리와 같이 밀린다). 실루엣 밖으로 나온 부분만 보인다 |
...
| 6.55 | 앞머리 — 코·안경 위, 눈썹·입 아래 (hairGroup) |
