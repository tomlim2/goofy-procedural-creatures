...
| armPose | rest/out/behind/up | 팔 기준각 + front/back 전환 |
...
형태(arms 슬롯)와 분리된 **동작**이다. `proportions.armRest`가 개체의 쉼 자세이고, clock이
그 자세를 기본으로 두다가 다른 자세로 갔다 돌아온다.

| 자세 | 기준각 (바깥 방향) | front/back |
| --- | --- | --- |
| rest | 0.35 | front |
| out | 1.35 | front |
| up | 2.4 | front |
| behind | −0.2 | **back** (팔꿈치만 삐죽) |

front/back 전환은 팔이 기준각 근처(0.35rad 이내)로 돌아온 뒤에만 한다. 회전 중 바꾸면 튄다.
매우 긴 팔(verylong)은 rest 자세를 후보에서 뺀다 — 늘어뜨리면 바닥을 뚫는다.
